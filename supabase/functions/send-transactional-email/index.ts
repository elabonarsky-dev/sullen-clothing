...
const SENDER_DOMAIN = "notify.sullenclothing.com"
// FROM_DOMAIN is the domain shown in the From: header (e.g., "example.com").
// When display_from_root is enabled, this can be the root domain for cleaner branding,
// even though actual sending uses the subdomain above.
const FROM_DOMAIN = "sullenclothing.com"
...
Deno.serve(async (req) => {
  ...
  // 5. Dedup check: skip if an email with this idempotencyKey was already sent/enqueued
  if (idempotencyKey && idempotencyKey !== messageId) {
    ...
    if ((existingCount ?? 0) > 0) {
      console.log('Dedup: skipping duplicate email', { templateName, effectiveRecipient, idempotencyKey })
      return new Response(
